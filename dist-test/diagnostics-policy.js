...
function normalizedFile(value) {
    ...
    if (sourceMarker)
        return sourceMarker.replace(/^\//, "");
    const distMarker = clean.match(/(?:^|\/)dist\/.+$/)?.[0];
    if (distMarker)
        return distMarker.replace(/^\//, "");
...
export function diagnosticPreview(error) {
    ...
    return {
        ...
        errorCode: safeText(source.code ? String(source.code) : "UNHANDLED_RUNTIME_ERROR", 100),
