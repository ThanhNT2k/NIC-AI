...
export function PortalHeader({active,user:providedUser}:{active?:PortalNavKey;user?:UserSummary}){
  ...
  async function openNotification(event:React.MouseEvent<HTMLAnchorElement>,item:NotificationItem){event.preventDefault();const href=notificationHref(item);setNotifications(current=>current.map(entry=>entry.id===item.id?{...entry,status:"read"}:entry));if(item.status!=="read")await fetch("/api/notifications",{method:"POST",headers:csrfHeaders(),body:JSON.stringify({action:"mark_read",id:item.id})});location.assign(href);}
