...
export async function PATCH(request:Request,context:{params:Promise<{id:string}>}){
  ...
  if(status==="completed"){
    const blockers=await db.prepare("SELECT COUNT(*) AS count FROM work_order_tasks WHERE work_order_id=? AND required=1 AND status<>'completed'").bind(id).first<{count:number}>();
    if((blockers?.count??0)>0)return Response.json({error:"MANDATORY_TASKS_INCOMPLETE"},{status:409});
    try{await db.batch([db.prepare("INSERT INTO work_order_close_approvals (id,work_order_id,requested_by,requested_at,status,note) VALUES (?,?,?,?,'pending',?)").bind(crypto.randomUUID(),id,user.id,now,resolution),db.prepare("INSERT INTO audit_logs (id,actor_id,action,entity_type,entity_id,metadata,created_at) VALUES (?,?,?,?,?,?,?)").bind(crypto.randomUUID(),user.id,"work_order.close_requested","maintenance_work_order",id,JSON.stringify({resolution}),now)]);}catch{return Response.json({error:"CLOSE_APPROVAL_ALREADY_PENDING"},{status:409});}
    return Response.json({workOrder:{id,status:"pending_close",assignedTo,updatedAt:now}},{status:202});
  }
