...
export async function GET(request:Request){
  ...
  const session=await createSession(user.id,{authMethod:"federated",mfaVerified}),headers=sessionHeaders(session);headers.set("Location",new URL(attempt.returnTo,request.url).toString());headers.set("cache-control","no-store");return new Response(null,{status:302,headers});
}
