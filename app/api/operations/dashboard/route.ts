...
export async function GET(request: Request) {
  ...
  if (user.departmentCode === "facility") {
    ...
    workOrders = (await db.prepare("SELECT w.id, w.request_id AS requestId, w.title, w.location, w.priority, w.status, w.assigned_to AS assignedTo, u.full_name AS assigneeName, w.provider_id AS providerId, p.name AS providerName, w.scheduled_at AS scheduledAt, w.resolution, w.updated_at AS updatedAt FROM maintenance_work_orders w LEFT JOIN users u ON u.id = w.assigned_to LEFT JOIN service_providers p ON p.id=w.provider_id ORDER BY CASE w.status WHEN 'open' THEN 0 WHEN 'scheduled' THEN 1 WHEN 'in_progress' THEN 2 ELSE 3 END, CASE w.priority WHEN 'critical' THEN 0 WHEN 'high' THEN 1 ELSE 2 END, w.updated_at DESC LIMIT 100").all()).results;
    providers = (await db.prepare("SELECT id,name,service_types AS serviceTypes FROM service_providers WHERE status='active' AND service_types LIKE '%maintenance%' ORDER BY name").all()).results;
  }
  return Response.json({ department: user.departmentCode, requests: requests.results, staff: staff.results, bookings, workOrders, providers });
}
